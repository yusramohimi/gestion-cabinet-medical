...
if (isset($_POST['rendez_vous_search'])) {
    $search = $_POST['rendez_vous_search'];

        
    $statement = $pdo->prepare(" SELECT rendez_vous.*, prestations.description, patients.nom 
        FROM rendez_vous 
...
$results_rendez_vous = $statement->fetchAll(PDO::FETCH_ASSOC);
ob_end_flush();
?>
        ...
        <div class='flex items-center justify-center w-full mt-4'>
            <div class="flex items-center rounded-full bg-[#0d1829] px-2 w-full max-w-[600px]">
                ...
                <form action="" method="POST" class="flex items-center w-full mb-3 mt-2">
                    <input type="search" class="w-full bg-transparent pl-2 text-[#cccccc] outline-0" placeholder="Rechercher un rendez-vous par nom de patient" name="rendez_vous_search" />
        ...
        <div class="p-4 xl:ml-80">
            <div class="mt-12">
                <button  class="flex gap-2 items-center  middle none center mr-4 rounded-lg bg-green-500 py-3 px-6 mb-5 font-sans text-xs font-bold uppercase text-white shadow-md shadow-green-500/20 transition-all hover:bg-green-600 hover:shadow-lg hover:shadow-green-500/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none" data-ripple-light="true">
                    ...
                    <a href="ajoutRv.php">Ajouter un rendez-vous</a>
                </button>
                <table class=" border-collapse w-full">
                    ...
                    <tbody>
                        <?php foreach($results_rendez_vous as $result_rendez_vous): ?>
                            <tr class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                                <td class="w-full lg:w-auto p-3 text-blue-600 underline font-bold text-center border border-b block lg:table-cell relative lg:static">
                                   ...
                                   <a href="detailsPatient.php?id=<?php echo $result_rendez_vous['id_patient'] ?>"><?php echo $result_rendez_vous['id_patient'] ?></a> 
                                </td>
                                ...
                                <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b text-center block lg:table-cell relative lg:static">
                                    ...
                                    <?php echo $result_rendez_vous['statut'] ?>
                                </td>
                                <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b text-center block lg:table-cell relative lg:static">
